...
int main(int argc, char **argv){
  ...
  while( ros::ok() ){
	...
	m.type = visualization_msgs::Marker::SPHERE;
	m.action = visualization_msgs::Marker::ADD;
	m.pose.position.x = HumanPoint.x;
	m.pose.position.y = HumanPoint.y;
	m.pose.position.z = HumanPoint.z;
	m.pose.orientation.y = HumanQuaternion.y;
	m.pose.orientation.z = HumanQuaternion.z;
	m.pose.orientation.w = HumanQuaternion.x; 
	m.scale.x = .2;
	m.scale.y = .2;
	m.scale.z = .2;
	m.color.r = 0.0f;
	...
	markers_pub = n.advertise<visualization_msgs::Marker>("marker2", 20);
